...
namespace Indiv0.BoxGame.Classes.Base
{
    class Terrain
    {
        ...
        public Terrain(string texString, /*int x, int y, */
            int w, int h,
        ...
        //            ii = i * _blockWidth;
        //            jj = j * _blockHeight;
        //            _textureMap[i, j] = new Sprite(
        //                "res/art/terrain/grass_",
        //                ii, jj, _blockWidth, _blockHeight);
        //        }
        //    }
        //}
        ...
        private float fractRand(float v) { return randomNum(-v, v); }

        private void GenerateHeightMap(/*int numberOfPasses*/)
        {         
            int _mapWidth = _arrayWidth;
            int _mapHeight = _arrayHeight;
            float _maxDisplacement = 1.0f;
            float _roughnessConstant = 0.1f;
            //int _numBoxes = 1;
            //int _passesSoFar = 0;
            //int _currentWidth;
            //int _currentHeight;
            _heightMap = new float[_blocksWide, _blocksHigh];

            _heightMap[0, 0] = fractRand(245);
            _heightMap[_arrayWidth, 0] = fractRand(532);
            _heightMap[0, _arrayHeight] = fractRand(34);
            _heightMap[_arrayWidth, _arrayHeight] = fractRand(432);

            //A = _heightMap[0, 0]
            //B = _heightMap[_arrayWidth, 0]
            //C = _heightMap[0, _arrayHeight]
            //D = _heightMap[_arrayWidth, _arrayHeight]

            //E = _heightMap[_arrayWidth / 2, _arrayHeight / 2]

            //F = _heightMap[0, _arrayHeight / 2]
            //G = _heightMap[_arrayWidth / 2, 0]
            //H = _heightMap[_arrayWidth, _arrayHeight / 2]
            //I = _heightMap[_arrayWidth / 2, _arrayHeight]

            //J = _heightMap[_arrayWidth / 4, _arrayHeight / 4]
            //K = _heightMap[(_arrayWidth / 4) * 3, _arrayHeight /4]
            //L = _heightMap[_arrayWidth / 4, (_arrayHeight / 4) * 3]
            //M = _heightMap[(_arrayWidth / 4) * 3, (_arrayHeight / 4) * 3]

            //E = (A+B+C+D) / 4 + RAND(d)
            _heightMap[_arrayWidth / 2, _arrayHeight / 2] = (_heightMap[0, 0] + _heightMap[_arrayWidth, 0]
                + _heightMap[0, _arrayHeight] + _heightMap[_arrayWidth, _arrayHeight]) / 4 + fractRand(_maxDisplacement);
            ...
            _heightMap[0, _arrayHeight / 2] = (_heightMap[0, 0] + _heightMap[0, _arrayHeight]
                + _heightMap[_arrayWidth / 2, _arrayHeight / 2]) / 3 + fractRand(_maxDisplacement);
            _heightMap[_arrayWidth / 2, 0] = (_heightMap[0, 0] + _heightMap[_arrayWidth, 0] 
                + _heightMap[_arrayWidth / 2, _arrayHeight / 2]) / 3 + fractRand(_maxDisplacement);
            _heightMap[_arrayWidth, _arrayHeight / 2] = (_heightMap[_arrayWidth, 0] + _heightMap[_arrayWidth, _arrayHeight] 
                + _heightMap[_arrayWidth / 2, _arrayHeight / 2]) / 3 + fractRand(_maxDisplacement);
            _heightMap[_arrayWidth / 2, _arrayHeight] = (_heightMap[0, _arrayHeight] + _heightMap[_arrayWidth, _arrayHeight]
                + _heightMap[_arrayWidth / 2, _arrayHeight / 2]) / 3 + fractRand(_maxDisplacement);

            //d *= pow(2,-r)

            _maxDisplacement *= (float)Math.Pow(2, -_roughnessConstant);
            ...
            _heightMap[_arrayWidth / 4, _arrayHeight / 4] = (_heightMap[0, 0] + _heightMap[_arrayWidth / 2, 0]
                + _heightMap[0, _arrayHeight / 2] + _heightMap[_arrayWidth / 2, _arrayHeight / 2]) / 4 + fractRand(_maxDisplacement);

            _heightMap[(_arrayWidth / 4) * 3, _arrayHeight / 4] = 
                (_heightMap[_arrayWidth / 2, 0] + _heightMap[_arrayWidth, 0]
                + _heightMap[_arrayWidth / 2, _arrayHeight / 2] + _heightMap[_arrayWidth, _arrayHeight / 2])
                / 4 + fractRand(_maxDisplacement);

            _heightMap[_arrayWidth / 4, (_arrayHeight / 4) * 3] =
                (_heightMap[0, _arrayHeight / 2] + _heightMap[_arrayWidth / 2, _arrayHeight / 2]
                + _heightMap[0, _arrayHeight] + _heightMap[_arrayWidth / 2, _arrayHeight]) / 4 + fractRand(_maxDisplacement);

            _heightMap[(_arrayWidth / 4) * 3, (_arrayHeight / 4) * 3] = 
                (_heightMap[_arrayWidth / 2, _arrayHeight / 2] + _heightMap[_arrayWidth, _arrayHeight / 2]
                + _heightMap[_arrayWidth / 2, _arrayHeight] + _heightMap[_arrayWidth, _arrayHeight]) / 4 + fractRand(_maxDisplacement);
            ...
            for (int j = 0; j <= _arrayHeight; j++)
            {
                for (int i = 0; i <= _arrayWidth; i++)
                {
                    if (_heightMap[i, j] != null)
                    {
                        ...
                        if (_heightMap[i, j] > 1 && _heightMap[i, j] < 2)
                        {
                            _textureMap[i, j] = new Sprite("res/art/terrain/grass_block", (i * _blockWidth), (j * _blockHeight),
                                _blockWidth, _blockHeight);
